...
func (t *Table2Struct) DDL () (ls map[string]string) {
	ls = make(map[string]string)
	t.dialMysql()
	if t.err != nil {
		fmt.Println(`error :`, t.err)
	}
	tables, _ := t.db.Query(`show tables `)
	defer tables.Close()
	show := shows
	tabs := tablex
	for tables.Next() {

		_ = tables.Scan(&show.TableInUnity)
		row, _ := t.db.Query("show create table " + show.TableInUnity)
		for row.Next() {
			_ = row.Scan(&tabs.Table, &tabs.CreateTable)
			ls[tabs.Table] = tabs.CreateTable
		}
	}
